...
struct TabMeta {
  ...
  bool is_index(const std::vector<std::string> &cols_name) const {
    for (auto &index : indexes) {
      if (index.col_num == cols_name.size()) {
        size_t i = 0;
        for (; i < index.col_num; ++i) {
          if (index.cols[i].name.compare(cols_name[i]) != 0) break;
        }
  ...
  std::vector<IndexMeta>::iterator get_index_meta(const std::vector<std::string> &col_names) {
    for (auto index = indexes.begin(); index != indexes.end(); ++index) {
      ...
      auto &index_cols = (*index /*deref the iterator*/).cols;
      size_t i = 0;
