...
std::shared_ptr<Plan> Planner::make_one_rel(std::shared_ptr<Query> query) {
    ...
    while (it != conds.end()) {
      ...
      if (left_need_to_join_executors != nullptr && right_need_to_join_executors != nullptr) {
        ...
        std::shared_ptr<Plan> temp_join_executors =
            std::make_shared<JoinPlan>(T_NestLoop, std::move(left_need_to_join_executors), std::move(right_need_to_join_executors), join_conds);
        table_join_executors =
            std::make_shared<JoinPlan>(T_NestLoop, std::move(temp_join_executors), std::move(table_join_executors), std::vector<Condition>());
  ...
  for (size_t i = 0; i < tables.size(); i++) {
    if (scantbl[i] == -1) {
      table_join_executors =
          std::make_shared<JoinPlan>(T_NestLoop, std::move(table_scan_executors[i]), std::move(table_join_executors), std::vector<Condition>());
    }
  }
...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan) {
  ...
  for (auto &col : all_cols) {
    if (col.name.compare(x->order->col->col_name) == 0) sel_col = {.tab_name = col.tab_name, .col_name = col.name};
  }
