...
struct ColDef : public Field {
  ...
  // 这里是 std::move, std::string 这里是传值, 实际上已经 Copy 了一次, 然后这里直接 Move, 减少了一次 Copy
...
struct OrderBy : public TreeNode {
  ...
  OrderByDir orderby_dir;  // enum OrderByDir { OrderBy_DEFAULT, OrderBy_ASC, OrderBy_DESC };
  OrderBy(std::shared_ptr<Col> cols_, OrderByDir orderby_dir_) : col(std::move(cols_)), orderby_dir(std::move(orderby_dir_)) {}
};
...
struct UpdateStmt : public TreeNode {
  ...
  UpdateStmt(std::string tab_name_, std::vector<std::shared_ptr<SetClause>> set_clauses_, std::vector<std::shared_ptr<BinaryExpr>> conds_)
      : tab_name(std::move(tab_name_)), set_clauses(std::move(set_clauses_)), conds(std::move(conds_)) {}
};

// select id, name, major, course, score from student
// join grade
...
// select id, name, major, course, score from student, grade
// where student.id = grade.student_id;
struct SelectStmt : public TreeNode {
  ...
  SelectStmt(std::vector<std::shared_ptr<Col>> cols_, std::vector<std::string> tabs_, std::vector<std::shared_ptr<BinaryExpr>> conds_,
...
extern std::shared_ptr<ast::TreeNode> parse_tree;  // from src/parser/ast.cpp

}  // namespace ast

#define YYSTYPE ast::SemValue  // alias of ast::SemValue
