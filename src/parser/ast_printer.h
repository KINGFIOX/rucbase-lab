...
class TreePrinter {
 ...
 private:
  static std::string offset2string(int offset) { return std::string(offset, ' '); }  // 打印缩进
  ...
  static std::string type2str(SvType type) {
    static const std::map<SvType, std::string> m{
        {SV_TYPE_INT, "INT"},
        {SV_TYPE_FLOAT, "FLOAT"},
  ...
  static std::string op2str(SvCompOp op) {
    static const std::map<SvCompOp, std::string> m{
  ...
  static void print_node(const std::shared_ptr<TreeNode> &node, int offset) {
    std::cout << offset2string(offset);
    offset += 2;
    // 如果是 rust 的话, 估计就使用 enum 和 match 了
    if (auto x = std::dynamic_pointer_cast<Help>(node)) {
      ...
      std::cout << "UPDATE\n";
      print_val(x->tab_name, offset);
      print_node_list(x->set_clauses, offset);  // print_node_list -> print_node, recursive
      print_node_list(x->conds, offset);
